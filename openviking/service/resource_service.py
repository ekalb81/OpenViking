...
class ResourceService:
    ...
    def _unsupported_connector_params(
        *,
        ...
        supported_args = CONNECTOR_SUPPORTED_ARGS.get(add_type, frozenset())
        ...
        **kwargs: Any,
        ...
        if wait:
            ...
            if outcome.get("status") != "completed":
                raise InternalError(
                    f"Connector import failed: {outcome.get('error') or 'unknown error'}"
                )
            response = {
                "status": "completed",
                "task_id": task.task_id,
    ...
    async def _monitor_connector_task(
        ...
        reason: str = "",
        link_root_uri: str = "",
        link_timeout: Optional[float] = None,
    ) -> Dict[str, Any]:
